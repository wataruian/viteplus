...
- **GitHub Actions**: `.github/workflows/main.yml` (push to `main`) and `.github/workflows/pr.yml` (pull requests into `main`) both call the reusable `.github/workflows/check.yml`, which matrixes over workspaces and runs `mise run dagger-artifact <fn> <pkg>` per workspace.
...
- **Self-hosted runner + persistent engine**: `check.yml` runs on `runs-on: [self-hosted, dagger]` instead of `ubuntu-latest`. Both the runner and a long-lived `dagger-engine` container are defined in `docker-compose.yml`; the runner's `_EXPERIMENTAL_DAGGER_RUNNER_HOST=docker-container://dagger-engine` env points every `dagger` call in a job at that shared engine, so cache volumes persist across CI runs instead of starting cold per job. The runner talks to Docker via the mounted host socket (docker-outside-of-docker, same pattern `dagger call load`/`publish` use locally) and re-registers after each job (`EPHEMERAL=1` + compose `restart: unless-stopped`).
