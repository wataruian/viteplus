...
- **GitHub Actions**: `.github/workflows/on-pull-request.yml` (pull requests into `main`) calls the reusable `.github/workflows/verify.yml`, which resolves the affected workspaces and fans out to `.github/workflows/verify-workspace.yml` per workspace, running `mise run dagger <fn> --workspace=<pkg> --export` — full verification (check/lint/type-check/test/Semgrep/Sonar) happens once, here, at the PR boundary. `.github/workflows/on-push-main.yml` (push to `main`) does not re-verify (branch protection requires the PR to be up to date first); it only runs `validate.yml` (commit-lint/CodeQL) then deploys to `dev` via the reusable `.github/workflows/deploy.yml`, which resolves the affected workspaces and fans out to `.github/workflows/deploy-workspace.yml` per workspace. Promoting a specific version to `staging`/`production` is a manual `.github/workflows/trigger.yml` dispatch naming a `version` (tag or full SHA), gated by each environment's required-reviewer rule; `.github/workflows/release.yml` cuts the version bump/tag/GitHub Release and deploys that commit to `dev`. Each deploy target (`dev`/`staging`/`production`) has its own concurrency group so a pending approval on one never blocks another.
...
- **Self-hosted runner + persistent engine**: `verify.yml`/`verify-workspace.yml` run on `runs-on: [self-hosted, dagger]` instead of `ubuntu-latest`. Both the runner and a long-lived `dagger-engine` container are defined in `docker-compose.yml`; the runner's `_EXPERIMENTAL_DAGGER_RUNNER_HOST=docker-container://dagger-engine` env points every `dagger` call in a job at that shared engine, so cache volumes persist across CI runs instead of starting cold per job. The runner talks to Docker via the mounted host socket (docker-outside-of-docker, same pattern `dagger call load`/`publish` use locally) and re-registers after each job (`EPHEMERAL=1` + compose `restart: unless-stopped`).
