...
- **GitHub Actions**: `.github/workflows/main.yml` (push to `main`) and `.github/workflows/pr.yml` (pull requests into `main`) both call the reusable `.github/workflows/ci.yml`, which matrixes over workspaces and runs `mise run dagger-artifact <fn> <pkg>` per workspace.
